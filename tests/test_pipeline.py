...
# -------------------------------------------------- issue -> exclusions robot

def _apply(body, tmp_path, monkeypatch, existing=""):
    """Run tools/apply_exclusions.py against an issue body, in a temp file."""
    import importlib, os
    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "tools"))
    mod = importlib.import_module("apply_exclusions")
    importlib.reload(mod)
    f = tmp_path / "exclusions.yaml"
    f.write_text(existing or "# header\n\nevents: []\nrecurring: []\n")
    monkeypatch.setattr(mod, "FILE", f)
    monkeypatch.setenv("ISSUE_BODY", body)
    monkeypatch.setenv("ISSUE_USER", "stella")
    monkeypatch.setenv("ISSUE_NUM", "7")
    ...
    try:
        ...
        pass
    import yaml as _y
    return _y.safe_load(f.read_text()) or {}, f.read_text()


def test_issue_robot_adds_requested_events(tmp_path, monkeypatch):
    doc, _ = _apply("""```yaml
events:
  - commission: ND
    date: 2026-08-26
...
```""", tmp_path, monkeypatch)
    assert doc["events"][0]["commission"] == "ND"
    assert doc["events"][0]["by"] == "stella"


def test_issue_robot_rejects_junk(tmp_path, monkeypatch):
    """It runs with write access on a public repo, so it accepts only the two
    keys it knows and validates every field."""
    doc, _ = _apply("""```yaml
events:
  - commission: NOT-A-CODE
    date: whenever
    title: nope
recurring:
  - commission: MD
    title_contains: xx
```""", tmp_path, monkeypatch)
    assert not doc.get("events") and not doc.get("recurring")


def test_issue_robot_is_idempotent(tmp_path, monkeypatch):
    """Submitting the same event twice must not duplicate it."""
    body = """```yaml
events:
  - commission: ND
    date: 2026-08-26
    title: Regular Meeting
```"""
    doc, text = _apply(body, tmp_path, monkeypatch)
    doc2, _ = _apply(body, tmp_path, monkeypatch, existing=text)
    assert len(doc2["events"]) == 1


def test_issue_robot_keeps_the_comment_header(tmp_path, monkeypatch):
    """Splitting on the word "events:" shredded the header, because it also
    appears in the comments explaining the file."""
    existing = "# how to use this\n# the events: key holds one-offs\n\nevents: []\nrecurring: []\n"
    _doc, text = _apply("""```yaml
events:
  - commission: ND
    date: 2026-08-26
    title: Regular Meeting
```""", tmp_path, monkeypatch, existing=existing)
    assert text.startswith("# how to use this")
    assert "# the events: key holds one-offs" in text
